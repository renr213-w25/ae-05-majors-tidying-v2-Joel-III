...
It is a chr variable since the column headings were coded as chr variables.It should be a numeric type instead.
...
*Add your response here.*
The NA means that value is empty in the dataframe since no student graduated with that degree that year
...
degree_type = fct_relevel(degree_type, "BS",
"BS2", "AB", "AB2")
)  
```
...
ggplot(mapping = aes(x = year, y = n, color = degree_type))+
  geom_line(aes(group=degree_type))+
  geom_point()+
  scale_x_discrete(breaks=c(2015,2020))
...
*Add your response here.*
We need to change the theme, axis labels, the x axis ticks, the line colors,position of the legend, add a title and subtitle and add the source at the bottom 
...
ggplot(mapping = aes(x = year, y = n, color = degree_type))+
  geom_line(aes(group=degree_type))+
  geom_point()+
  scale_x_discrete(breaks=seq(2011,2023,2))
```
